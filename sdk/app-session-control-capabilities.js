...
export const APP_SESSION_SWITCH_MODEL_CAPABILITY = "app/session.switch-model";
/** Gates `session:update`'s and `session:create`'s `thinkingLevel` field for a verified v2 caller. */
export const APP_SESSION_THINKING_LEVEL_CAPABILITY = "app/session.thinking-level";
/** Gates `session:update`'s and `session:create`'s `permissionMode` field for a verified v2 caller. */
export const APP_SESSION_PERMISSION_MODE_CAPABILITY = "app/session.permission-mode";
/** Gates `session:send` and any `session:send-custom` whose message will enter the model. */
export const APP_SESSION_START_TURN_CAPABILITY = "app/session.start-turn";
/** Reserved for changing a specified session's next-run tool set; no control behavior is opened here. */
export const APP_SESSION_TOOLS_CONFIGURE_CAPABILITY = "app/session.tools.configure";
/**
 ...
 */
export const APP_SESSION_CONTROL_CAPABILITY_WORDS = Object.freeze([
    APP_SESSION_SWITCH_MODEL_CAPABILITY,
    APP_SESSION_THINKING_LEVEL_CAPABILITY,
    APP_SESSION_PERMISSION_MODE_CAPABILITY,
    APP_SESSION_START_TURN_CAPABILITY,
    APP_SESSION_TOOLS_CONFIGURE_CAPABILITY,
]);
//# sourceMappingURL=app-session-control-capabilities.js.map